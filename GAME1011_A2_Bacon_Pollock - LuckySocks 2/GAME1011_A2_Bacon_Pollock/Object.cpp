...
Object::Object(std::string name, Object* parent) : m_sName(name), m_pParent(parent), m_bLoaded(false), m_bCloser(true) {}

Object::~Object() {}

// SAM: Inventory conditionals
bool hasCuriousLittleKey, hasFamilyOfDolls, hasHairs, hasFingernails, hasTeeth = false;
			...
			else
			{
				...
				if (!valid)
				{
					for (std::list<Object*>::iterator it = m_pChildren.begin(); it != m_pChildren.end(); it++)
					{
						...
						{
							// SAM: Conditional progress checks.
								// SAM: Conditionals for opening Curious Little Door
							if ((input == "look at curious little lock"))
							{
								if (hasCuriousLittleKey)
								{
									std::cout << "The curious little key fits right in.\n";
								}
								else
								{
								...
								// Conditionals for determining ending.
							else if (input == "look at toy chest")
							{
								if (hasFamilyOfDolls)
								{
							...
							else if (input == "look at curious little girl") // SAM ADDED
							{
									// Check for parts.
								if (hasFingernails)
								{
									std::cout << "She takes Fingernails\n";
								}
								if (hasTeeth)
								{
									std::cout << "She takes Teeth\n";
								}
								if (hasHairs)
								{
									std::cout << "She takes hairs\n";
								}
						...
						else if (input == pickstr)
						{
							if ((*it)->GetAttached() == false)
							{
								...
								valid = true;
								// Conditionals for flagging picked up items
							if ((input == "pick up curious little key")) 
							{
								hasCuriousLittleKey = true;
							}
							if ((input == "pick up hairs"))
							{
								hasHairs = true;
							}
							if ((input == "pick up teeth")) 
							{
								hasTeeth = true;
							}
							if ((input == "pick up fingernails")) 
							{
								hasFingernails= true;
							}
							if ((input == "pick up family of dolls")) 
							{
								hasFamilyOfDolls = true;
							}
								break;
							}
							else
...
bool Object::m_LoadFromFile()
{
	...
	std::fstream file;
	std::string filename = "Objects/" + m_sName + ".txt";
